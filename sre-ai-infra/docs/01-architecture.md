...
| Layer | Choice |
|---|---|
| Frontend | React (Vite or Next.js — final choice made in `sre-ai-frontend` repo), Material Design via MUI |
...
Three separate git repositories (decision recorded in [[00-overview]]):

- **`sre-ai-frontend`** — the React app only.
- **`sre-ai-backend`** — FastAPI app, LangGraph agents, ingestion services, DB migrations.
- **`sre-ai-infra`** *(this repo)* — `docs/` (all requirement/architecture markdown, source of truth), `docker-compose/` for local dev, `eks/` + `terraform/` for prod deployment manifests.

See [[07-repo-structure-and-conventions]] for exactly how these three repos reference each other and how a Claude Code session working in `sre-ai-frontend` or `sre-ai-backend` should pull in this repo's docs.
...
- **Prod**: AWS EKS (backend + frontend as separate deployments/services, or frontend on S3+CloudFront — decide in `sre-ai-infra`), AWS RDS for Postgres+pgvector, LLM = AWS Bedrock.
