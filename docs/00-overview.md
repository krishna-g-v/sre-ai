...
- Ops runbook/change-assistant (exact-step guidance for tasks like EKS upgrades or new deployments), multi-account read-only AWS access via AssumeRole, ECR/network monitoring tools, cross-app incident timelines, and proactive EKS pod-breakdown alerting — added after MVP, still advisory-only. See [[09-ops-runbook-and-proactive-monitoring]].
...
| Multi-account AWS access | AssumeRole from one central identity (dev: operator's AWS CLI profile; prod: IRSA role) into a per-account curated-read-only IAM role — not static per-account access keys, not the broad AWS-managed `ReadOnlyAccess` policy. See [[09-ops-runbook-and-proactive-monitoring]] §2. |
| Product/incident-timeline grouping | Admin-configured `product_groups` mapping only, consistent with "no auto-discovery" elsewhere — not k8s-label inference, not ad hoc per-question lists. See [[09-ops-runbook-and-proactive-monitoring]] §5. |
| Ops runbook/change-assistant output | Exact CLI/eksctl/kubectl command steps grounded in real discovered state — no generated manifest/IaC files in this phase. See [[09-ops-runbook-and-proactive-monitoring]] §4. |
| Proactive pod-breakdown monitoring | In scope now (not deferred), reusing the existing `Agent`/`Alert`/AI-triage pipeline rather than a new one; in-app alert delivery only for now (Slack/email deferred but additive later). Default triggers: CrashLoopBackOff/restart spike, OOMKilled, Failed/Pending stuck. See [[09-ops-runbook-and-proactive-monitoring]] §7. |
| NL-to-CLI command execution | LLM generates real `aws`/`kubectl` commands (RAG few-shot, text-to-SQL-style), gated by a deterministic read-only verb allowlist + scope check before execution — complements, doesn't replace, the hand-written tool catalog in [[09-ops-runbook-and-proactive-monitoring]] §3. Secrets/data-plane reads (Secrets Manager, SSM decrypt, KMS, S3 objects, RDS log content) are excluded even though technically "read" operations. See [[10-nl-to-cli-execution]]. |
| NL-to-CLI account access | Per-user, self-service, not admin-provisioned — each user registers their own AWS account role ARNs under Settings → AWS Accounts (superusers use the same screen for their own accounts, no special-casing). Only this pipeline works this way; the fixed tool catalog in [[09-ops-runbook-and-proactive-monitoring]] stays on admin-provisioned group scopes. See [[10-nl-to-cli-execution]] §9. |
| VPC network topology visualization | A new fixed tool (boto3, not a NL-to-CLI generated command — needs ~10 coordinated `describe-*` calls cross-referenced into one structure), reusing per-user `user_aws_accounts` access. Given a starting VPC, renders subnets/route tables/gateways/instances plus one hop into directly-connected peered VPCs, Site-to-Site VPN, and Transit-Gateway-attached VPCs (same account/region only for v1) as a Mermaid diagram + text summary. Routed via a dedicated LLM classification call in `live_ops_node`, not a keyword match. Built, unit-tested (142 backend tests passing), and verified live end to end against a real account (2026-09-15) — a real bug (the LLM drawing its own fabricated diagram despite being told not to) was found and fixed with a deterministic strip, not just a stronger prompt. See [[11-network-topology-visualization]]. |
...
| [09-ops-runbook-and-proactive-monitoring.md](09-ops-runbook-and-proactive-monitoring.md) | Ops runbook/change-assistant (exact-step EKS upgrade/deployment guidance), multi-account AssumeRole access, ECR/network monitoring tools, cross-app incident timelines, proactive EKS pod-breakdown alerting |
| [10-nl-to-cli-execution.md](10-nl-to-cli-execution.md) | Natural-language-to-AWS-CLI/kubectl command generation and execution (text-to-SQL-style), the read-only validator/allowlist gate that replaces "no mutating tool exists" as the guardrail for this pathway, and per-request credential isolation |
| [11-network-topology-visualization.md](11-network-topology-visualization.md) | Given a starting VPC, discover and render its network topology (subnets, gateways, peering, Site-to-Site VPN, Transit Gateway attachments, instances) as a Mermaid diagram — design doc, not yet implemented |
