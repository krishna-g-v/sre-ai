...
4. **Knowledge Base management** — per-user view of: their personal documents (upload/delete), and documents in each group they belong to (upload if permitted, browse/search always). Upload flow lets the user pick target scope (personal vs. a specific group they belong to), add free-form tags, and choose the chunking strategy for that document — "whole document" or "best effort" with a small/medium/large qualitative size, defaulting to the system-wide setting (see [[02-access-control-and-rag]] §3) — plus an optional "Convert to Markdown before ingesting" checkbox (disabled for files that are already Markdown) that runs the raw extracted text through the LLM to clean it up before chunking (see [[02-access-control-and-rag]] §3a). Documents converted this way show a "Markdown-converted" badge in the document list.
